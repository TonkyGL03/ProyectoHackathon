...
export function ReminderCard({ medication, time, dosage, type, instructions, patient, onClick }: ReminderCardProps) {
  const getTypeStyles = () => {
    switch (type) {
      case "taken":
        return {
        ...
        <div className="flex items-start justify-between">
          <div className="flex items-start space-x-3 flex-1">
            <div className={`p-2 rounded-full bg-card ${styles.iconColor}`}>
              {type === "overdue" ? <AlertCircle size={20} /> : <Pill size={20} />}
            </div>
            <div className="flex-1">
              <div className="flex items-center space-x-2 mb-1">
                ...
                <span className="text-xs text-muted-foreground">• Hab. {patient.room}</span>
              </div>
